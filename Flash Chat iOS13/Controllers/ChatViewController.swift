...
class ChatViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // title = K.appName already done in xcode
        ...
        db.collection(K.FStore.collectionName)
            ...
            else
            {
                if let snapshotDocuments = querySnapchot?.documents {
                    ...
                    {
                        // ici on crée notre data qu'on va envoyer a firebase
                        let data = doc.data()
    ...
    @IBAction func sendPressed(_ sender: UIButton) {
        if let messageBody = messageTextfield.text,
            ...
            { error in
                if let e = error {
                    print("There was an issues saving data to firestore, \(e)")
                } else {
                    print("Successefuly saved data.")
                }
            }
            DispatchQueue.main.async {
